<div align="center">

# Jose Manuel Veiga
### Unity Developer · Bedrock Dev · Software Engineer

[![Portfolio](https://img.shields.io/badge/Portfolio-joseveiga.dev-black?style=flat-square&logo=astro)](https://joseveiga.dev/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-joseveigac-0077B5?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/joseveigac/)
[![CurseForge](https://img.shields.io/badge/CurseForge-ghozix-F16436?style=flat-square&logo=curseforge)]([https://www.curseforge.com/minecraft-bedrock/addons/idle-ore-generators](https://www.curseforge.com/members/ghozix/projects))

</div>

---

I build games and software tools with a focus on clean architecture and real shipping. Experienced with Unity across commercial Steam titles, Android, and cloud backends. Currently building solo projects and exploring multi-platform publishing.

---

### ✅ Released

| Project | Description | Stack |
|---|---|---|
| [**Creator Socials**](https://creatorsocials.es/) | Multi-platform publisher for creators — upload once, post to YouTube, TikTok & Instagram with scheduling and analytics | Web, OAuth, API |
| [**Idle Ore Generators**](https://www.curseforge.com/minecraft-bedrock/addons/idle-ore-generators) | Published Minecraft Bedrock add-on. Idle ore generator items via BP/RP packs | `@minecraft/server` 2.4.0 |
| [**Trainpunk Run**](https://store.steampowered.com/app/1501560/Trainpunk_Run/) | Side-scrolling action game shipped on Steam — contributed to architecture and release | Unity, C# |
| [**My Daily Pets**](https://store.steampowered.com/app/1476020/My_Daily_Pets/) | Casual pet simulation game shipped on Steam — refactored codebase applying SOLID principles | Unity, C# |
| [**Portfolio & Dev Blog**](https://joseveiga.dev/) | Personal site with development notes and guides | Astro |

---

## Experience

**Addon Developer** — Aurrora Studio (Current)*
Developing Minecraft Bedrock add-ons as part of the studio team.                                              

**Minecraft Marketplace Developer** — Pathway Studios *(Jan 2026 – Present)*
Collaborating with an official Minecraft Marketplace Partner on Marketplace-ready content. Developing Bedrock add-ons using Script API and TypeScript.

**Unity Developer** — Vision Brain S.L *(2024)*
Upgraded Android project to API 30+, resolved AWS backend instability, updated full toolchain (JRE, JDK, .NET, DLLs).

**Unity Developer (Remote)** — Jellyworld Games *(2021 – 2022)*
Delivered two Steam titles. Refactored architecture with OOP and SOLID principles in an Agile remote team.

**Unity Developer (Internship)** — Jellyworld Games *(2021)*
Supported multiple Unity projects from development through launch.

---

## Tech

<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/csharp/csharp.png" alt="C#"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/unity/unity.png" alt="Unity"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/android/android.png" alt="Android"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/javascript/javascript.png" alt="JavaScript"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/firebase/firebase.png" alt="Firebase"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/aws/aws.png" alt="AWS"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/git/git.png" alt="Git"></code>

`Unity 6` `C#` `TypeScript` `JavaScript` `Android` `Firebase` `AWS` `SQLite` `MongoDB` `Unreal Engine` `C++` `Java` `Git` `Agile/Scrum`

---

<div align="center">

[![GitHub Stats](https://github-readme-stats.vercel.app/api?username=joseveigac&show_icons=true&theme=github_dark&hide_border=true&include_all_commits=true)](https://github.com/joseveigac)
[![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=joseveigac&layout=compact&theme=github_dark&hide_border=true)](https://github.com/joseveigac)

</div>
