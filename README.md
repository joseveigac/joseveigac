<div align="center">

# Jos Manuel Veiga
### Unity Developer · Addon Dev · Software Engineer

[![Portfolio](https://img.shields.io/badge/Portfolio-joseveiga.dev-black?style=flat-square&logo=astro)](https://joseveiga.dev/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-joseveigac-0077B5?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/joseveigac/)
[![CurseForge](https://img.shields.io/badge/CurseForge-ghozix-F16436?style=flat-square&logo=curseforge)](https://www.curseforge.com/minecraft-bedrock/addons/idle-ore-generators)

</div>

---

I build games and software tools with a focus on clean architecture and real shipping. Experienced with Unity across commercial Steam titles, Android, and cloud backends. Currently building solo projects and exploring multi-platform publishing.

---

## Tech

<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/csharp/csharp.png" alt="C#"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/unity/unity.png" alt="Unity"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/android/android.png" alt="Android"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/javascript/javascript.png" alt="JavaScript"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/firebase/firebase.png" alt="Firebase"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/aws/aws.png" alt="AWS"></code>
<code><img height="20" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/git/git.png" alt="Git"></code>

`Unity 6` `C#` `Addressables` `Android` `Firebase` `AWS` `SQLite` `MongoDB` `Unreal Engine` `Godot` `C++` `Java` `JavaScript` `Git` `Agile/Scrum`

---

### ✅ Released

| Project | Type | Engine / Stack | Description |
|---|---|---|---|
| [**Trainpunk Run**](https://store.steampowered.com/app/1501560/Trainpunk_Run/) | 🎮 PC Game · Steam | Unity · C# | Side-scrolling action game. Contributed to architecture, refactoring, and release as part of a remote Agile team. |
| [**My Daily Pets**](https://store.steampowered.com/app/1476020/My_Daily_Pets/) | 🎮 PC Game · Steam | Unity · C# | Casual pet simulation game. Refactored codebase applying OOP and SOLID principles before launch. |
| [**Creator Socials**](https://creatorsocials.es/) | 🌐 Web App | Web · REST API · OAuth | Multi-platform publishing tool — upload once, schedule and post to YouTube, TikTok & Instagram with cross-platform analytics. |
| [**Idle Ore Generators**](https://www.curseforge.com/minecraft-bedrock/addons/idle-ore-generators) | 🧩 Bedrock Add-on · CurseForge | `@minecraft/server` 2.4.0 · TypeScript | Published Minecraft Bedrock add-on adding idle ore generator items via Behavior and Resource packs. |
| [**Portfolio & Dev Blog**](https://joseveiga.dev/) | 🌐 Static Site | Astro 5 | Personal site with development notes, project writeups, and guides. |

---

### Experience
<table>
  <tr>
    <td align="center" width="90">
      <a href="https://aurrora.net">
        <img src="Aurrora-logo-Black-not-stacked.svg" width="64" alt="Aurrora Studio" />
      </a>
    </td>
    <td>
      <strong>Addon Developer</strong><br>
      Aurrora Studio · <em>Current · Remote</em><br>
      Developing Minecraft Bedrock add-ons as part of the studio team.
    </td>
  </tr>
    <tr>
    <td align="center" width="90" bgcolor="white">
      <a href="https://pathway.studio"><img src="https://joseveiga.dev/img/pathway_logo.webp" width="64" alt="Pathway Studios" /></a>
    </td>
    <td>
      <strong>Minecraft Marketplace Developer</strong><br>
      <a href="https://pathway.studio">Pathway Studios</a> · <em>Jan 2026 – Present · Remote</em><br>
      Developing Marketplace-ready Bedrock add-ons for an official Minecraft Marketplace Partner. Using Script API and TypeScript.
    </td>
  </tr>
  <tr>
    <td align="center" width="90" bgcolor="white">
      <a href="#"><img src="https://joseveiga.dev/img/brainvt_logo.webp" width="64" alt="BrainVT" /></a>
    </td>
    <td>
      <strong>Unity Developer – Android API & Backend</strong><br>
      BrainVT · <em>Jun 2024 – Aug 2024</em><br>
      Upgraded Android project to API 30+, resolved AWS backend instability, updated full toolchain (JRE, JDK, .NET, DLLs).
    </td>
  </tr>
  <tr>
    <td align="center" width="90" bgcolor="white">
      <a href="https://jellyworld.games"><img src="https://joseveiga.dev/img/jellyworld_logo.webp" width="64" alt="Jellyworld Games" /></a>
    </td>
    <td>
      <strong>Unity Developer (Remote)</strong><br>
      <a href="https://jellyworld.games">Jellyworld Games</a> · <em>May 2021 – Jun 2022</em><br>
      Shipped Trainpunk Run and My Daily Pets on Steam. Refactored architecture with OOP and SOLID principles across Agile sprints.
    </td>
  </tr>
  <tr>
    <td align="center" width="90" bgcolor="white">
      <a href="https://jellyworld.games"><img src="https://joseveiga.dev/img/jellyworld_logo.webp" width="64" alt="Jellyworld Games" /></a>
    </td>
    <td>
      <strong>Unity Developer (Internship)</strong><br>
      <a href="https://jellyworld.games">Jellyworld Games</a> · <em>Jan 2021 – Apr 2021</em><br>
      Supported multiple Unity projects with clean, structured code and launch prep.
    </td>
  </tr>
</table>
---

<div align="center">

[![GitHub Stats](https://github-readme-stats.vercel.app/api?username=joseveigac&show_icons=true&theme=github_dark&hide_border=true&include_all_commits=true)](https://github.com/joseveigac)
[![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=joseveigac&layout=compact&theme=github_dark&hide_border=true)](https://github.com/joseveigac)

</div>
